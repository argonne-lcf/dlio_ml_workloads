...
class Metric:
    ...
    def __del__(self):
        ...
        self.summary['compute_time'] = [[m for m in l[:self.steps]] for l in self.compute_time[:self.epochs]]
        self.summary['step_time'] = [[m for m in l[:self.steps]] for l in self.step_time[:self.epochs]]
        print(self.summary)
        if get_rank()==0:
            self.logging("============================================================")
