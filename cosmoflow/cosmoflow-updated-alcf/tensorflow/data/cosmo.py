...
def construct_dataset(file_dir, n_samples, batch_size, n_epochs,
                      ...
                      n_parallel_reads=tf.data.AUTOTUNE, prefetch=tf.data.AUTOTUNE, cache=None):
    ...
    if cache!=None:
        if (cache == "RAM"):
            data = data.apply(wrap_dataset).cache().map(parse_data, num_parallel_calls=n_parallel_reads)
        else:
            data = data.apply(wrap_dataset).cache(cache).map(parse_data, num_parallel_calls=n_parallel_reads)
    else:
