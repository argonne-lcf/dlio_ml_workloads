...
#PBS -q preemptable
#qsub -A datascience -q preemptable run-job-4node-large-data.sc
...
aprun -n 16 -N 4 python ./main.py +mpi.local_size=4 ++data.stage=/local/scratch/ +log.timestamp=ms_4node_360mins +log.experiment_id=${PBS_JOBID} --config-name test_tfr 
